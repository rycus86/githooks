...
If you want to install from another repository (e.g. from your own fork), you can specify the release repository url 
as well as the branch name (default: `master`) when installing with:
```shell
sh -c "$(curl -fsSL https://r.viktoradam.net/githooks)" -- --release-clone-url "https://server.com/my-githooks-fork.git" --release-clone-branch "release"
