...
if ! [ -f "$GH_TEST_TMP/git-templates/templates/hooks/pre-commit" ]; then
    ...
    exit 1
elif ! grep 'github.com/rycus86/githooks' "$GH_TEST_TMP/git-templates/templates/hooks/pre-commit"; then
...
if ! grep 'github.com/rycus86/githooks' "$GH_TEST_TMP/test6/.git/hooks/pre-commit"; then
