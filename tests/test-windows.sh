...
sed -i 's|</dev/tty||g' "$ROOT_DIR"/install.sh &&
    ...
    sed -i -E 's|GITHOOKS_HOOK_FOLDER=(.*)\\\|GITHOOKS_HOOK_FOLDER=\1 GITHOOKS_HOOK_FOLDER="\${HOOK_FOLDER:-\$GITHOOKS_HOOK_FOLDER}" \\\|' "$ROOT_DIR"/base-template-wrapper.sh &&
    sed -i -E 's|GITHOOKS_HOOK_NAME=(.*)\\\|GITHOOKS_HOOK_NAME=\1 GITHOOKS_HOOK_NAME="\${HOOK_NAME:-\$GITHOOKS_HOOK_NAME}" \\\|' "$ROOT_DIR"/base-template-wrapper.sh &&
