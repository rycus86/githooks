...
fetch_latest_updates() {
    ...
    update_release_clone --fetch-only || return 1
...
#   Sets `$GITHOOKS_CLONE_DIR_IS_UPDATED`.
#
...
#####################################################
execute_update() {
    if [ -n "$GITHOOKS_UPDATE_COMMIT" ]; then
        # We still have a merge to do...
    ...
    if DO_UPDATE_ONLY="true" sh <"$INSTALL_SCRIPT"; then
